...
def init_db():
    ...
    # Recommendations table for dynamic recommendations
...
def seed_default_recommendations():
    ...
    if count == 0:
        ...
        for level, recs in default_recommendations.items():
            for rec in recs:
                c.execute('''INSERT INTO recommendations (id, stress_level, title, description, icon, created_at, updated_at)
                            VALUES (?, ?, ?, ?, ?, ?, ?)''',
                          (str(uuid.uuid4()), level, rec['title'], rec['description'], rec['icon'], now, now))
        
        conn.commit()
        print("✓ Default recommendations seeded successfully!")
...
# ============================================
# RECOMMENDATIONS ENDPOINTS (Dynamic)
# ============================================

@app.route('/api/all-recommendations', methods=['GET'])
def get_all_recommendations():
    """Get all recommendations grouped by stress level"""
    conn = get_db()
    c = conn.cursor()
    
    c.execute('''SELECT id, stress_level, title, description, icon FROM recommendations 
                 ORDER BY stress_level, created_at''')
    recs = c.fetchall()
    conn.close()
...
def get_recommendations(stress_level):
    """Get personalized stress management recommendations from database"""
    level = stress_level.capitalize()
    
    if level not in ['Low', 'Medium', 'High']:
        return jsonify({'error': 'Invalid stress level. Use Low, Medium, or High'}), 400
    
    conn = get_db()
    c = conn.cursor()
    
    c.execute('''SELECT id, title, description, icon FROM recommendations 
                 WHERE stress_level = ? ORDER BY created_at''', (level,))
    recs = c.fetchall()
    conn.close()
    
    recommendations = [{'id': r['id'], 'title': r['title'], 'description': r['description'], 'icon': r['icon']} for r in recs]
...
def add_recommendation():
    ...
    if not data.get('stress_level') or not data.get('title') or not data.get('description'):
        return jsonify({'error': 'stress_level, title, and description are required'}), 400
    
    level = data['stress_level'].capitalize()
    if level not in ['Low', 'Medium', 'High']:
        return jsonify({'error': 'Invalid stress level. Use Low, Medium, or High'}), 400
    ...
    c.execute('''INSERT INTO recommendations (id, stress_level, title, description, icon, created_at, updated_at)
                 VALUES (?, ?, ?, ?, ?, ?, ?)''',
              (rec_id, level, data['title'], data['description'], icon, now, now))
...
def update_recommendation(rec_id):
    ...
    stress_level = data.get('stress_level', existing['stress_level'])
    ...
    c.execute('''UPDATE recommendations 
                 SET title = ?, description = ?, icon = ?, stress_level = ?, updated_at = ?
                 WHERE id = ?''',
              (title, description, icon, stress_level, now, rec_id))
